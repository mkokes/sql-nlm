...
export async function POST(request: Request) {
  ...
  if (
    ...
    results = mockOrders.filter(
      o =>
        new Date(o.order_date) > new Date('2023-04-01') &&
        new Date(o.order_date) < new Date('2023-05-01')
    );
    ...
    const customerSpending = mockCustomers
      .map(customer => {
