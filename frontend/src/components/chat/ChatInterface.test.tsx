  ...
  return {
    Select: ({ children, onValueChange }: any) => {
      ...
      return (
        <>
          {children}
          <div data-testid="select-item-1" onClick={() => onValueChangeMock(1)}>
            E-commerce Database
          </div>
          <div data-testid="select-item-2" onClick={() => onValueChangeMock(2)}>
            Blog System
          </div>
        </>
      );
    ...
    SelectItem: ({ value, children }: any) => (
      <div data-testid={`select-item-${value}`} onClick={() => onValueChangeMock(value)}>
        {children}
      </div>
    ),
  };
...
describe('ChatInterface component', () => {
  ...
  it('sends a query and displays the response', async () => {
    ...
    fireEvent.change(inputField, {
      target: { value: 'Show me all products with price greater than $100' },
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Show me all products with price greater than $100')
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText('SELECT * FROM products WHERE price > 100 ORDER BY price DESC')
      ).toBeInTheDocument();
    });
  });
  ...
  it('handles API errors when submitting a query', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Sorry, there was an error processing your query/i)
      ).toBeInTheDocument();
    });
